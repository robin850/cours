...
Un tableau va, dans la plupart des cas, contenir en ensemble d'éléments qui ont un ou plusieurs points en communs. Ça peut ne pas être le cas bien évidemment mais par logique on essaye de garder une certaine logique. Pour créer un tableau (vide pour le moment), il vous suffit d'utiliser un crochet fermant et entrant :
...
Plutôt simple, non ? Si vous ne conceptualisez pas immédiatement l'intérêt d'un tableau, ça sera sans doute plus clair au fil du chapitre. Voici donc d'autres exemples de tableaux :
...
Pour faire appel aux différents éléments d'un tableau, un facteur indispensable entre en jeu : les indexes (aussi appelé indices parfois). Les indexes d'un tableau sont numériques et représentent la position d'un élément dans un tableau. Contrairement à ce que l'on pourrait penser, le premier index __n'est pas__ l'index 1 mais 0.
...
# L'élément n'existe pas, ruby renvoie nil (nul)
puts langages[6] # => nil
~~~
...
L'exemple le plus courant est l'utilisation de la méthode `to_a` sur des éléments (comme les chaînes de caractères). Le comportement peut sembler quelques peux étrange au départ mais pour les chaînes de caractères mais pour convertir une chaîne de caractères en tableau, il faut _chaîner_ `to_a` avec la méthode `chars` :
...
C'est le cas le plus courant d'utilisation de la méthode `to_a`. On peut également construire un tableau d'une certaine longueur où chaque élément est déjà prédéfini comme ceci par exemple :
...
Histoire que vous compreniez un peu mieux quand et surtout comment utiliser un tableau, voici une liste de méthodes qui pourra vous être utile dès que vous aurez affaire à des tableaux. Ce type d'exemples vous aidera sans doute à mieux conceptualiser ce qu'est un tableau et quand l'utiliser.

### Additionner et soustraire des tableaux

L'exemple le plus basique que l'on peut montrer avec les tableaux, est l'addition de plusieurs tableaux ensemble :
...
fruits_et_legumes = ["pomme", "orange", "citron", "noix de coco", "courgette", "carotte", "salade"]
...
Plusieurs moyens s'offrent à nous quand à l'insertion et à la suppression de données dans un tableau. Nous allons ici voir les opérations les plus courantes. Ce sera sans doute le genre d'opération que vous effectuerez le plus.
...
L'avantage de ce genre de méthodes est que l'on peut par exemple créer un tableau vide et réaliser un traitement en remplissant le tableau une fois ce traitement fait. Par exemple, voici un bout de code qui remplit un tableau ne contenant que les prénoms commençant par la lettre « j » :
...
Maintenant que nous avons vu comment ajouter des valeurs aux tableaux, regardons comment en supprimer. La méthode la plus simple est sans doute `pop`. Elle ne prend aucun argument ou peut éventuellement prendre le nombre d'éléments qu'elle doit supprimer.
...
La méthode `uniq` renvoie uniquement le tableau une fois les doublons supprimés, elle ne modifie pas le tableau lui même. Si vous affichez ensuite la valeur du tableau `fruits`, les doublons seront toujours présents. Pour modifier le tableau en supprimant les doublons, il faut faire appel à la méthode `uniq!` (avec un point d'exclamation). Les méthodes de ce genre sont appelées méthodes _bang_. Nous parlerons de leur fonctionnement encore une fois dans la seconde partie.

### Mélanger notre tableau

Une méthode plutôt amusante est la méthode `shuffle`. Cette dernière permet d'organiser de manière aléatoire les éléments de votre tableau. Le résultat n'est donc pas toujours le même :
...
La méthode `sort` peut également s'éverer très utile. Son effet est en quelques sortes inverse à celui de `shuffle`. Elle permet d'ordonner notre tableau selon l'ordre numérique ou alphabétique. Par exemple :
...
Nous n'en avons pas vraiment parlé mais il est bien sûr possible de mettre un tableau dans un tableau. Voir même que ce tableau contiennent un autre tableau. La méthode `flatten` vient à notre secours lorsqu'il s'agit de ne créer qu'un seul tableau avec les différents éléments contenus dans les différents tableaux imbriqués. Par exemple :
...
Très simple à appréhender et très pratique aussi !
...
Par la suite, vous aurez besoin de tirer une valeur au hasard dans un de vos tableaux. C'est là que la méthode `sample` intervient.
...
Pour finir, une méthode très simple que l'on avait déjà vu en parlant des chaines de caractères : `length`. La méthode `length` permet de connaitre le nombre d'éléments dans un tableau.
...
Avec cette petite liste de méthodes, vous avez maintenant quelques outils pour travailler correctement avec les tableaux. Bien sûr, d'autres méthodes seront vues par la suite.
...
Pour parcourir un tableau, la méthode la plus courante est sans doute la méthode `each`. On peut également utiliser `for` mais son utilisation est plutôt rare ; on lui préfère `each`. Évidemment, dans les deux cas, le résultat est le même.
...
En fait, `for` fait appel à la méthode `each` de l'élément passer après le `in`. Nous verrons plus tard que ça peut être très utile.
...
~~~
Si certains se demandent l'intérêt d'une telle démarche, on peu imaginer par la suite remplacer (par le biais des indexes) toutes les occurrences de `"John"` par un autre prénom par exemple. D'autres cas sont aussi envisageables.
...
Pendant la réalisation du programme, pensez bien aux types. `rand` renvoie un nombre (_Integer_) et `gets` renvoie une chaine de caractères (_String_). Pensez-y ! La méthode `p` peut également être votre amie !

Une fois le programme réalisé, même s'il fonctionne et qu'il répond à toutes les contraintes, il y a fort à parier que le code ne soit pas le même que celui que nous allons vous proposer. C'est bien ! Cela veut dire que vous avez essayer de comprendre et de progresser !
...
end while ["oui", "o", "y", "yes"].include?(continuer.downcase)
