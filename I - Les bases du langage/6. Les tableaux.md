...
Nous avons vu précédemment que Ruby nous procurait des méthodes pour la conversion des éléments. La méthode de conversion vers les tableaux est `to_a`, cependant pour convertir une chaîne de caractères en un tableau, il faut utiliser la méthode `chars` :
